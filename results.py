...
#--------------------------------------------------------CHANGED----------------------------------------------------------#
        ...
        if pred_boxes and show_boxes:
            for d in reversed(pred_boxes):
                ...
                # Display bounding boxes for class 0
                # if c == 0:
                #
                ...
                if c == 0:
                    ...
                    # Extract the region of interest (ROI) for blue object detection
                    roi = frame[int(y1):int(y2), int(x1):int(x2)]

                    # Detect blue object and draw green segmentation
                    blue_object_contour = detect_blue_object(roi, lower_blue, upper_blue)
                    if blue_object_contour is not None:
                        # Draw the green segmentation boundaries using the detected contour
                        contour_to_draw = blue_object_contour.copy()
                        contour_to_draw[:, :, 0] -= int(x1)
                        contour_to_draw[:, :, 1] -= int(y1)
                        green_color = (0, 255, 0)  # Adjust the color here (BGR format)
                        cv2.drawContours(roi, [contour_to_draw], 0, green_color, 1)  # Green segmentation boundaries

                        # Calculate the height of the segmented region
                        touching_arm_heights = calculate_touching_arm_heights(blue_object_contour, 0, 0, int(x2 - x1),
                                                                              int(y2 - y1), num_pixels=15)
                        if len(touching_arm_heights) >= 2:
                            # cv2.putText(frame, f"Height (Left Touching Arm): {touching_arm_heights[1]} pixels", (0, 90),
                            #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
                            # cv2.putText(frame, f"Height (Right Touching Arm): {touching_arm_heights[0]} pixels",
                            #             (0, 120),
                            #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
                            cv2.putText(frame, f"Height: {min(touching_arm_heights)} pixels", (0, 90),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                        else:
                            cv2.putText(frame, "Height: N/A", (0, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255),
                                        1)
                # Calculate and display bounding box area for class 1
                if c == 1:
                    # Calculate bounding box area using d.xyxy values
                    x1, y1, x2, y2 = d.xyxy.squeeze()
                    x_res = x2 - x1
                    y_res = y2 - y1

                    bbox_area = x_res * y_res

                    label = ''  # You can add class names with f'{names[c]} '
                    #min_height_from_arms= min(touching_arm_heights[0],touching_arm_heights[1])

                    # Append the area information to the label
                    if labels:
                        label += f'Area: {(bbox_area / (grid_spacing * grid_spacing))*diameter:.0f} mm'
